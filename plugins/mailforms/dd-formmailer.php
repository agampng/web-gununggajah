...
/*
Dagon Design Form Mailer  Version 5.8 
released 2009
http://www.dagondesign.com/articles/secure-php-form-mailer-script/

adapted for CMSimple Plugin MailForms 1.6
updated for php5 and higher
released 02/2016
http://www.ge-webdesign.de/cmsimpleplugins/?Eigene_Plugins/MailForms
...
if(isset($_REQUEST['mailform']))
{
	$ddmfContactPageAdd = '&mailform';
}
else
{
	$ddmfContactPageAdd = '';
...
$path_contact_page = "./?" . $u[$s] . $ddmfContactPageAdd . "#formmail";
$path_contact_page_sent = "./?" . $u[$s] . $ddmfContactPageAdd . "&ddfmFormSent#formmail";
...
$sender_name = 'Pesan dari form kontak';
$sender_email = 'ddmf_sender_email';

if(!isset($ddmfSubject) || $ddmfSubject == '')
{
	$email_subject = 'Pesan dari ' . $_SERVER['SERVER_NAME'] . $sn;
}
else
{
...
$message_structure = '';
$sent_message = '<p class="ddmfMailSent">' . $plugin_tx['mailforms']['frontend_email_sent'] . '</p>';
...
$verify_background = '222';  // hex code for background color
$verify_text = '999'; // hex code for text color
$force_type = 'jpeg'; // problems showing the code? try forcing to 'gif', 'jpeg' or 'png'
...
function ddfm_send_mail($recipients, $sender_name, $sender_email, $email_subject, $email_msg, $attach_save, $attach_path, $attachments = false) 
{
	...
	$headers .= 'From: ' . $sender_name . ' <' . $ddfm_email_from . '>' . PHP_EOL;
...
if (isset($_POST["form_submitted"])) 
{
	...
	if ($errors) 
	{
		$ddfmOutput.= '<div class="ddfmwrap"><div class="ddfmerrors">' . DDFM_ERRORMSG . '</div>';
		$ddfmOutput.= '<div class="errorlist">';
		foreach ($errors as $err) 
		{
			$ddfmOutput.= $err . '<br />';
		}
		$ddfmOutput.= '</div><div style="clear:both;"><!-- --></div></div>';
...
{
	$ddfmOutput.= $sent_message;
	$ddfmOutput.= '<p class="ddfm_backToForm"><a href="' . $path_contact_page . '">' . $plugin_tx['mailforms']['frontend_back_to_form'] . '</a></p>';
}
...
$ddfmOutput.= '<!-- END of Dagon Design Formmailer output -->' . "\n\n\n";
