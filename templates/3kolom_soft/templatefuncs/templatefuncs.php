...
// tf_prevpageTitle
function tf_prevpageTitle($before='&laquo; ',$after='') 
{
	global $pth, $tx, $s, $cl, $tx, $adm, $h, $u, $sn, $title;
	for ($i = $s; $i > -1; $i--)
		if (!hide($i-1))
		{
			if($s >= 1)
			{
				return '<a href="' . $sn . '?' . $u[$i-1] . '" title="' . $tx['navigator']['previous'] . '">' . $before . $h[$i-1] . $after . '</a>';
			}
			else
			{
				return $tx['locator']['home'];
			}
		}
	if($s < 0) return $title;
}
...
// tf_nextpageTitle
function tf_nextpageTitle($before='',$after=' &raquo;') 
{
	global $pth, $tx, $s, $cl, $tx, $adm, $h, $u, $sn;
	for ($i = $s + 1; $i < $cl; $i++)
		if (!hide($i))
		{
			return '<a href="' . $sn . '?' . $u[$i] . '" title="' . $tx['navigator']['next'] . '">' . $before . $h[$i] . $after . '</a>';
		}
}
